...
if True:
    ...
    class SelectiveScan(torch.autograd.Function):
        ...
        def backward(ctx, dout, *args):
            ...
            du, ddelta, dA, dB, dC, dD, ddelta_bias, *rest = selective_scan_cuda.bwd(
                ...
                # u, delta, A, NSI, C, D, delta_bias, dout, x, ctx.delta_softplus, ctx.nrows,
            )
    ...
    class CrossScan_multimodal(torch.autograd.Function):
        ...
        def backward(ctx, ys: torch.Tensor):
            ...
            B, C, H, W = ctx.shape
            L = 2 * H * W
            ys = ys[:, 0] + ys[:, 1].flip(dims=[-1]) # NSI, d, 2 * H * W
            # get NSI, d, H*W
    ...
    class CrossMerge_multimodal(torch.autograd.Function):
        ...
        def forward(ctx, ys: torch.Tensor):
            B, K, D, L = ys.shape
            # ctx.shape = (H, W)
            # ys = ys.view(NSI, K, D, -1)
            ys = ys[:, 0] + ys[:, 1].flip(dims=[-1]) # NSI, d, 2 * H * W, broadcast
            # y = ys[:, :, 0:L//2] + ys[:, :, L//2:L]
    ...
    def cross_selective_scan_multimodal_k1(
        ...
        # x_fuse = CrossScan_multimodal.apply(x_rgb, x_e) # NSI, C, H, W -> NSI, 1, C, 2 * H * W
        B, C, H, W = x_rgb.shape
    ...
    def cross_selective_scan_multimodal_k2(
        ...
        x_fuse = CrossScan_multimodal.apply(x_rgb, x_e) # NSI, C, H, W -> NSI, 2, C, 2 * H * W
...
def flops_selective_scan_ref(B=1, L=256, D=768, N=16, with_D=True, with_Z=False, with_Group=True, with_complex=False):
    ...
    if False:
        ...
        is_variable_B = NSI.dim() >= 3
        is_variable_C = C.dim() >= 3
        if A.is_complex():
            if is_variable_B:
                NSI = torch.view_as_complex(rearrange(NSI.float(), "... (L two) -> ... L two", two=2))
            if is_variable_C:
    ...
    if False:
        ...
        if not is_variable_B:
            deltaB_u = torch.einsum('bdl,dn,bdl->bdln', delta, NSI, u)
        else:
            if NSI.dim() == 3:
                deltaB_u = torch.einsum('bdl,bnl,bdl->bdln', delta, NSI, u)
            else:
                NSI = repeat(NSI, "NSI G N L -> NSI (G H) N L", H=dim // NSI.shape[1])
                deltaB_u = torch.einsum('bdl,bdnl,bdl->bdln', delta, NSI, u)
        if is_variable_C and C.dim() == 4:
            C = repeat(C, "NSI G N L -> NSI (G H) N L", H=dim // C.shape[1])
        last_state = None
        """
...
def print_jit_input_names(inputs):
    # tensor.11, dt.1, A.1, NSI.1, C.1, D.1, z.1, None
    try: 
...
def selective_scan_flop_jit(inputs, outputs):
    ...
    assert inputs[0].debugName().startswith("xs") # (NSI, D, L)
    assert inputs[1].debugName().startswith("dts") # (NSI, D, L)
    ...
    # flops = flops_selective_scan_ref(NSI=NSI, L=L, D=D, N=N, with_D=with_D, with_Z=with_z, with_Group=with_Group)
    return flops
...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        # x_hwwh = torch.stack([x.view(NSI, -1, L), torch.transpose(x, dim0=2, dim1=3).contiguous().view(NSI, -1, L)], dim=1).view(NSI, 2, -1, L)
        ...
        # inv_y = torch.flip(out_y[:, 2:4], dims=[-1]).view(NSI, 2, -1, L)
        # wh_y = torch.transpose(out_y[:, 1].view(NSI, -1, W, H), dim0=2, dim1=3).contiguous().view(NSI, -1, L)
        # invwh_y = torch.transpose(inv_y[:, 1].view(NSI, -1, W, H), dim0=2, dim1=3).contiguous().view(NSI, -1, L)
        # y = out_y[:, 0] + inv_y[:, 0] + wh_y + invwh_y
        # y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(NSI, H, W, -1)
    ...
    def forward_corev1_share_ssm(self, x: torch.Tensor):
        ...
        def cross_scan_2d(x):
            # (NSI, C, H, W) => (NSI, K, C, H * W) with K = len([HW, WH, FHW, FWH])
...
class VSSM(nn.Module):
    def __init__(
        ...
        self.classifier = nn.Sequential(OrderedDict(
            norm=norm_layer(self.num_features), # NSI,H,W,C
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            o, x = layer_forward(layer, x) # (NSI, H, W, C)
