...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, extra_emb=None):  # hidden_states=extra_emb: (1,16384,32)
        """
        hidden_states: (NSI, L, D)
        ...
        if extra_emb is None:
            ...
            hidden_states = self.in_proj1(hidden_states)
            extra_emb = self.in_proj2(extra_emb)
            x_dbl_rgb = self.x_proj_1(rearrange(hidden_states, "b l d -> (b l) d"))  # (bl d)
            x_dbl_e = self.x_proj_2(rearrange(extra_emb, "b l d -> (b l) d"))  # (bl d)
            ...
            # dt_rgb = rearrange(dt_rgb, "d (b l) -> b d l", l=seqlen)
            ...
            B_rgb = rearrange(B_rgb, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
            B_e = rearrange(B_e, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
            C_rgb = rearrange(C_rgb, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                out = mamba_inner_fn_no_out_proj(
                    ...
                    self.conv1d.weight,
                    self.conv1d.bias,
                    self.x_proj_1.weight,
                    self.dt_proj_1.weight,
                    A_rgb,
                    B_rgb,  # input-dependent NSI
                    C_e,  # input-dependent C
                    self.D.float(),
                    delta_bias=self.dt_proj.bias.float(),
                    delta_softplus=True,
                ...
                if extra_emb is not None:
                    out_b = mamba_inner_fn_no_out_proj(
                        ...
                        self.conv1d_b.weight,
                        self.conv1d_b.bias,
                        self.x_proj_2.weight,
                        self.dt_proj_2.weight,
                        A_e,
                        B_e,
                        C_rgb,
                        self.D_b.float(),
                    ...
                    # F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias)
                    if not self.if_devide_out:
                        out_r = F.linear(rearrange(out, "b d l -> b l d"), self.out_proj1.weight, self.out_proj1.bias)
                        out_d = F.linear(rearrange(out_b, "b d l -> b l d"), self.out_proj2.weight, self.out_proj2.bias)

                    else:
                        out = F.linear(rearrange(out + out_b, "b d l -> b l d") / 2, self.out_proj.weight, self.out_proj.bias)
                else:
                    out_r = rearrange(out, "b d l -> b l d")
            ...
            if conv_state is not None:
                ...
                conv_state.copy_(F.pad(x, (self.d_conv - x.shape[-1], 0)))  # Update state (NSI D W)
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        xz = self.in_proj(hidden_states.squeeze(1))  # (NSI 2D)
        x, z = xz.chunk(2, dim=-1)  # (NSI D)

        # Conv step
        if causal_conv1d_update is None:
            conv_state.copy_(torch.roll(conv_state, shifts=-1, dims=-1))  # Update state (NSI D W)
            conv_state[:, :, -1] = x
            x = torch.sum(conv_state * rearrange(self.conv1d.weight, "d 1 w -> d w"), dim=-1)  # (NSI D)
        ...
        x_db = self.x_proj(x)  # (NSI dt_rank+2*d_state)
        dt, B, C = torch.split(x_db, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        # Don't add dt_bias here
        dt = F.linear(dt, self.dt_proj.weight)  # (NSI d_inner)
        A = -torch.exp(self.A_log.float())  # (d_inner, d_state)

        # SSM step
        if selective_state_update is None:
            # Discretize A and NSI
