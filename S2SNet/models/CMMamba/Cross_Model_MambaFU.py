...
import numbers
from CMMamba.mamba_simpleFU import Mamba
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x,x_size):
        B,HW,C = x.shape
        x = x.transpose(1, 2).view(B, self.nc, x_size[0], x_size[1])  # NSI Ph*Pw C
        return x
...
class CrossMamba(nn.Module):
    ...
    def forward(self, ms, ms_resi, pan):  # ms (1,16384,32), pan (1,16384,32), ms_resi=0
        ...
        B, HW, C = global_f.shape  # NSI:1, HW:16384,C:32)
        H = W = int(math.sqrt(HW))
        # ms = global_f.transpose(1, 2).view(NSI, C, 128*8, 128*8)
...
class CrossMamba_(nn.Module):
    def __init__(self, dim):
        ...
        self.norm2 = LayerNorm(dim, 'with_bias')
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim)  # depth-wise convolution
        self.dwconv2 = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim)  # depth-wise convolution  # depth-wise convolution

    def forward(self, ms, pan):  # ms (1,16384,32), pan (1,16384,32)

        out_r, out_d = self.cross_mamba(self.norm1(ms), extra_emb=self.norm2(pan))  # global_f (1,16384,32)
        rgb = out_r + ms  # shortcut connection
        dep = out_d + pan



        B, HW, C = rgb.shape  # NSI:1, HW:16384,C:32)

        H = W = int(math.sqrt(HW))
        rgb = rgb.transpose(1, 2).view(B, C, H, W)  # Reshape: (1,16384,32)-->(1, 32, 128,128)
        dep = dep.transpose(1, 2).view(B, C, H, W)  # Reshape: (1,16384,32)-->(1, 32, 128,128)
        rgb = self.dwconv(rgb)
        dep = self.dwconv2(dep)

        # ms = self.dwconv(ms).flatten(2).transpose(1, 2)  # (1, 128,128, 32)-->(1,16384,32)
        return rgb, dep

class SinMamba_(nn.Module):
    def __init__(self, dim):
        super(SinMamba_, self).__init__()
        self.cross_mamba = Mamba(dim, bimamba_type="v3")
        self.norm = LayerNorm(dim, 'with_bias')
        self.dwconv = DWConv(dim, dim)  # depth-wise convolution

    def forward(self, x):  # ms (1,16384,32), pan (1,16384,32)

        out = self.cross_mamba(self.norm(x))  # global_f (1,16384,32)
        NSI, HW, C = out.shape  # NSI:1, HW:16384,C:32)
        H = W = int(math.sqrt(HW))
        out = out.transpose(1, 2).view(NSI, C, H, W)  # Reshape: (1,16384,32)-->(1, 32, 128,128)
        out = self.dwconv(out)
